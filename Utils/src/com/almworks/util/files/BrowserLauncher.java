...
class BrowserLauncher {
  ...
  /**
   * The message from any error thrown throughout the initialization process.
   */
  ...
  static {
    ...
    if (osName.startsWith("Mac OS")) {
      ...
      String majorMRJVersion = mrjVersion != null && mrjVersion.length() >=3 ? mrjVersion.substring(0, 3) : "";
      try {
   ...
   */
  private static Object locateBrowser() throws InterruptedException {
    if (browser != null) {
      return browser;
    }
    switch (jvm) {
    case MRJ_2_0:
      try {
        ...
        // locateBrowser(), we'll access the same AppleEvent, to which we'll already have
    ...
    case OTHER:
    default:
      String[] browsers = {"xdg-open", "gnome-open", "firefox", "google-chrome", "chromium-browser", "opera", "konqueror", "mozilla", "netscape"};
      String b = null;
      for (int count = 0; count < browsers.length && b == null; count++) {
        try {
   ...
   * @param encoded if true, the url is already encoded
   * @throws IOException If the web browser could not be located or does not run
   */
  public static void openURL(String url, boolean encoded) throws IOException, InterruptedException {
    ...
    case MRJ_2_0:
      ...
      try {
        ...
        aeDesc = null;        // Encourage it to access disposed if it was created
        browser = null;        // Ditto
      }
      break;
    case MRJ_2_1:
    ...
    case MRJ_3_1:
      ...
      break;
    case WINDOWS_NT:
      String urlParam = encoded ? url : encodeUrl(url);
      Process processNt = Runtime.getRuntime().exec(new String[] {"rundll32", "url.dll,FileProtocolHandler", urlParam});
      try {
        processNt.waitFor();
        processNt.exitValue();
    ...
    case WINDOWS_9x:
      ...
      process = Runtime.getRuntime()
        .exec(new String[] {(String) browser, FIRST_WINDOWS_PARAMETER, SECOND_WINDOWS_PARAMETER,
      ...
      break;
    case OTHER:
      // Assume that we're on Unix
/*
      String[] browsers = {"firefox", "opera", "konqueror", "mozilla", "netscape"};
      browser = null;
      for (int count = 0; count < browsers.length && browser == null; count++) if (Runtime.getRuntime()
        .exec(new String[]{"which", browsers[count]})
        .waitFor() ==
        0) browser = browsers[count];
      if (browser == null) throw new Exception("Could not find web browser."); else Runtime.getRuntime().exec(
...
*/
    ...
    if (process != null) {
      ...
      if (code != 0) {
        throw new IOException("cannot launch browser: code " + code);
      }
    }
  }
  ...
  private static String encodeUrl(String urlString) {
    if (urlString == null)
      return null;
    try {
      org.apache.commons.httpclient.URI uri = new org.apache.commons.httpclient.URI(urlString, false, "UTF-8");
      return uri.getEscapedURIReference();
    } catch (URIException e) {
      Log.warn("cannot parse url " + urlString);
      return urlString;
    }
  }
