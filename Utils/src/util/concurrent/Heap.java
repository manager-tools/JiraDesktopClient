...
public class Heap <T> {
  ...
  public synchronized void insert(T x) {
    if (count_ >= nodes_.length)
      ensureCapacity(3 * nodes_.length / 2 + 1);
  ...
  public synchronized void ensureCapacity(int heapSize) {
    if (nodes_.length < heapSize) {
      Object[] newnodes = new Object[heapSize];
      System.arraycopy(nodes_, 0, newnodes, 0, nodes_.length);
      nodes_ = newnodes;
    }
  }

  public synchronized Object[] copyUnsortedContents() {
    Object[] array = new Object[nodes_.length];
    System.arraycopy(nodes_, 0, array, 0, nodes_.length);
    return array;
  }
}
