...
public class DecentFormatter extends Formatter {
  ...
  public String format(LogRecord record) {
    ...
    result.append(DecentFormatter.LINE_SEPARATOR);
    Throwable thrown = record.getThrown();
    if (thrown != null)
      DecentFormatter.printStackTrace(thrown, result, 0);
  ...
  public static void printStackTrace(Throwable thrown, StringBuffer result, int descent) {
    try {
      ...
      DecentFormatter.printStackTrace(thrown.getStackTrace(), result);

      Throwable cause = thrown.getCause();
      if (cause != null) {
        result.append("Caused by: ");
        DecentFormatter.printStackTrace(cause, result, descent + 1);
      }
      if (descent == 0) {
        result.append("==========================================================================================");
        result.append(DecentFormatter.LINE_SEPARATOR);
  ...
  public static final class QuickDateFormatter {
    ...
    public void formatTo(long time, StringBuffer appendTo) {
      int utime = (int) (time / 1000);
      int millis = (int) (time - utime * 1000);
      int day = utime / 86400;
    ...
    private void appendInt(int v, StringBuffer appendTo, int digitNumber) {
      ...
      if (digitNumber >= 3) {
        ...
        appendTo.append(DecentFormatter.QuickDateFormatter.ourDigits[digit]);
        v -= digit * 100;
      }
      digit = v / 10;
      if (digit > 9)
      ...
      if (v > 9)
        throw new IllegalArgumentException("" + v);
      appendTo.append(DecentFormatter.QuickDateFormatter.ourDigits[v]);
    }
  }
}
